...
export default function RegisterPage() {
  ...
  const validate = () => {
    const e = {};
    if (!NAME_REGEX.test(form.name)) e.name = 'Name must be 2–40 letters only.';
    if (!EMAIL_REGEX.test(form.email)) e.email = 'Enter a valid email address.';
    if (!PASSWORD_REGEX.test(form.password)) e.password = 'Password doesn\'t meet all requirements below.';
    return e;
  };
              ...
              {pwChecks.map((c) => {
                ...
                return (
                  <div key={c.label} style={s.pwRule}>
                    <span style={{ color: ok ? '#3EB87A' : '#C0C0BE', fontSize: '11px' }}>
                      {ok ? '✓' : '○'}
                    </span>
